...
//SELECT EASY and HARD buttons using class now
const modeButtons = document.querySelectorAll(".mode")

//REFACTOR for Diffculty buttons
//means we can add medium, etc. later
for(let i = 0; i < modeButtons.length; i++) {
  modeButtons[i].addEventListener("click", function(){
    //HARD CODE TAKE AWAY CLASS
    ...
    //USE TURINARY OPERATOR SAME AS THIS CODE
    /*
    ...
    */
   //Turinary: if === Easy ?then (first conditiong) :otherwise second condition
...
function reset() {
  //copied from resetButton
    ...
    //change when hit play again
    messageDisplay.textContent = "";
    //change colors of squares on page | like below
    for(let i = 0; i < squares.length; i++){
      //hide other 3
      //if there is a color to paint, else we will do somehting else
      if(colors[i]){
        //bring back colors for hard mode
        squares[i].style.display = "block";
        //print colors for squares
        squares[i].style.backgroundColor = colors[i];
      } else {
        //display none if no color to paint
        squares[i].style.display = "none";
      }
    }
    h1.style.backgroundColor = "steelblue"
    // resetButton.textContent = "New Colors"; add this above instead
